...
(function() {
  ...
  angular.module('ng-extra', ['ngResource']).run(function() {
    return angular.clean = function(obj) {
      return angular.fromJson(angular.toJson(obj));
    };
  }).run([
    '$rootScope', function($rootScope) {
      $rootScope.$watchOnce = function(watchExpression, listener, objectEquality) {
        var deregistrater;
        return deregistrater = $rootScope.$watch(watchExpression, function() {
          deregistrater();
          return listener != null ? typeof listener.apply === "function" ? listener.apply.apply(listener, [this].concat(__slice.call(arguments))) : void 0 : void 0;
        }, objectEquality);
      };
      return $rootScope.$watchCollectionOnce = function(watchExpression, listener) {
        var deregistrater;
        return deregistrater = $rootScope.$watchCollection(watchExpression, function() {
          deregistrater();
          return listener != null ? typeof listener.apply === "function" ? listener.apply.apply(listener, [this].concat(__slice.call(arguments))) : void 0 : void 0;
        });
      };
      ...
      return $provide.decorator('$resource', [
        '$delegate', function($delegate) {
          return function(url, paramDefaults, actions) {
            ...
            Resource.wrapStaticMethod = function(fnName, fn, deleteInstanceMethod) {
              var originalFn;
              if (deleteInstanceMethod == null) {
                deleteInstanceMethod = true;
              }
              originalFn = Resource[fnName];
              if (deleteInstanceMethod) {
                delete Resource.prototype["$" + fnName];
              }
              return Resource[fnName] = fn(function() {
                return originalFn.apply(Resource, arguments);
              });
            };
            return Resource;
          };
      ...
      return $provide.decorator('$resource', [
        '$delegate', function($delegate) {
          return function(url, paramDefaults, actions) {
            var Resource;
            Resource = $delegate(url, paramDefaults, actions);
            angular.forEach(actions, function(options, method) {
              var retainprops, _ref;
              if (!options.normalize) {
                return;
              }
              retainprops = (_ref = options.retainprops) != null ? _ref : ['id'];
              return Resource.prototype["$" + method] = function(params, data, success, error) {
                var errorHandler, result, successHandler;
                ...
                angular.forEach(retainprops, (function(_this) {
                  return function(property) {
                    return data[property] = _this[property];
                  };
                })(this));
                successHandler = (function(_this) {
                  return function(resp, headers) {
                    _this.$resolved = true;
                    angular.copy(angular.clean(resp), _this);
                    return typeof success === "function" ? success(_this, headers) : void 0;
                  };
                })(this);
                errorHandler = function(resp) {
                  this.$resolved = true;
                  return typeof error === "function" ? error(resp) : void 0;
                };
                this.$resolved = false;
    ...
    '$q', '$parse', function($q, $parse) {
      return {
        link: function(scope, element, attrs) {
          ...
          isBusy = false;
          changeMethod = element.is('input') ? 'val' : 'text';
          ...
          bindEvents = function(eventNames) {
            var $form, event, events, normalEvents, submitEvents, _i, _len;
            submitEvents = [];
            normalEvents = [];
            events = eventNames.split(' ');
            for (_i = 0, _len = events.length; _i < _len; _i++) {
              event = events[_i];
              (/^submit(\.|$)?/.test(event) ? submitEvents : normalEvents).push(event);
            }
            ...
            return scope.$on('$destroy', function() {
              return $form.off(submitEvents.join(' '), handler);
            });
          };
          bindPromise = function(promiseName) {
            return scope.$watch(promiseName, function(promise) {
          ...
          return scope.$watch((function() {
            ...
            element["" + (isBusy ? 'add' : 'remove') + "Class"]('disabled');
            element["" + (isBusy ? 'a' : 'removeA') + "ttr"]('disabled', 'disabled');
            if (isBusy && angular.isDefined(attrs.busybtnText)) {
              return element[changeMethod](attrs.busybtnText);
            } else if (originalText != null) {
              return element[changeMethod](originalText);
            }
          });
        }
      };
      ...
      return {
        ...
        link: function(scope, element, attrs, ngModel) {
          ...
          return scope.$watch((function() {
            ...
            element["" + (isBusy ? 'add' : 'remove') + "Class"]('disabled');
            element["" + (isBusy ? 'a' : 'removeA') + "ttr"]('disabled', 'disabled');
            if (isBusy && angular.isDefined(attrs.busyboxText)) {
              return element.val(attrs.busyboxText);
            } else if (ngModel) {
              return element.val(ngModel.$modelValue);
            }
          });
        }
      };
    ...
    '$window', '$q', function($window, $q) {
      ...
      $dialog = {};
      methods = {
        alert: function(defer, result) {
          return defer.resolve();
        },
        confirm: function(defer, result) {
          var deferMethod;
          deferMethod = result ? 'resolve' : 'reject';
          return defer[deferMethod](result);
        },
        prompt: function(defer, result) {
          var deferMethod;
          deferMethod = result != null ? 'resolve' : 'reject';
          return defer[deferMethod](result);
        }
      };
      angular.forEach(methods, function(handler, name) {
        return $dialog[name] = function(options) {
          var defer, result;
          defer = $q.defer();
...
(function() {
  var debounce, difference, entryMap, originalRemove, originalResult,
  ...
  _.mixin({
    mixPatch: function(map) {
      return _(map).chain().keys().filter(function(key) {
        return __indexOf.call(_.keys(_), key) < 0;
      }).map(function(key) {
    ...
    disjunctor: function(signal, handlerMap, options) {
      ...
      return _.result(context, handler, args);
    },
    equalChecker: function(obj, deepEqual) {
      var simpleEqual;
      simpleEqual = function(a, b) {
        return a === b;
      };
      return _.partial((deepEqual ? _.isEqual : simpleEqual), obj);
    },
    difference: function() {
    ...
    escape: function(string, ignoreChar) {
      ...
      _.each(ignoreChar, function(char) {
        return _.arrayDel(keys, char, true);
      });
    ...
    unescape: function(string, ignoreChar) {
      ...
      keys = _.keys(entryMap.escape);
      _.each(ignoreChar, function(char) {
        return _.arrayDel(keys, entryMap.escape[char], true);
      });
      return ('' + string).replace(RegExp("[" + (keys.join('')) + "]", "g"), function(match) {
    ...
    pack: function(obj) {
      ...
      if (!_.every(obj, function(value) {
        return _.isArray(value);
      })) {
        return obj;
      }
      result = [];
      _.forEach(obj, function(vals, key) {
        return _.forEach(vals, function(value, index) {
          result[index] || (result[index] = _.isArray(obj) ? [] : {});
    ...
    walk: function(array, property, callback, thisArg) {
      if (!_.isArray(array)) {
        return;
      }
      if (!property) {
      ...
      return _.forEach(array, function(elem) {
        callback.call(thisArg, elem, array);
        return _.walk(elem[property], property, callback, thisArg);
      });
    },
    destRemove: originalRemove,
    remove: function(array, filter, option, thisArg) {
      var newArray, targetElems;
      if (filter == null) {
        filter = _.identity;
      }
      if (option == null) {
      ...
      if (_(option).isBoolean()) {
        option = {
          destructive: option
        };
      }
      if (_(filter).isObject() || _(filter).isFunction()) {
        targetElems = _.filter(array, filter, thisArg);
      } else {
        targetElems = [filter];
      }
      newArray = array;
      _.each(targetElems, function(elem) {
        if (!option.destructive) {
          newArray = newArray.slice();
        }
        return originalRemove.call(_, newArray, _.equalChecker(elem));
      });
      return newArray;
    },
    ...
    arrayDel: function() {
      ...
      return _.remove.apply(_, arguments);
    },
    debounce: function(handler, wait, options) {
      var func;
      ...
      if (!_(options.argsProcesser).isFunction()) {
        return func;
      }
      return function() {
        var args;
        args = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        return func.apply(this, options.argsProcesser(args));
      };
    }
  });
  ...
  _.mixin({
    ...
    result: function(object, property, args, context) {
      ...
      if (arguments.length === 2) {
        return originalResult(object, property);
    ...
    hasProp: function() {
      ...
      return _.hasProps.apply(_, arguments);
    },
    hasProps: function(obj, props, some) {
      return _(props).chain().map(_.partial(_.has, obj)).resultWithArgs((some ? "some" : "every"), _.identity).value();
    },
    sum: function(array) {
      if (!_.isArray(array)) {
        return;
      }
      return _.reduce(array, function(result, number) {
    ...
    findIndex: function(array, callback, thisArg) {
      ...
      if (_.isFunction(callback)) {
        _.forEach(array, function(elem, index) {
          if (callback.call(thisArg, elem)) {
            return index;
          }
        });
        return -1;
      } else if (_.isObject(callback)) {
        return _.indexOf(array, _.findWhere(array, callback));
      } else {
        return _.pluck(array, callback);
      }
    }
  });
